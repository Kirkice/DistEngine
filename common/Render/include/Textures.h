...
namespace render
{
	...
	void SaveTextureAsEXR(const TextureData<Vector4>& texture, const wchar* filePath);
	...
	template<typename T> static DirectX::XMVECTOR SampleTexture2D(Vector2 uv, uint32 arraySlice, const Array<T>& texels,
		uint32 texWidth, uint32 texHeight, uint32 numSlices)
	{
		Vector2 texSize = Vector2(float(texWidth), float(texHeight));
		Vector2 halfTexelSize(0.5f / texSize.x, 0.5f / texSize.y);
		Vector2 samplePos = Frac(uv - halfTexelSize);
		if (samplePos.x < 0.0f)
		...
		Vector2 lerpAmts = Vector2(Frac(samplePos.x), Frac(samplePos.y));
	...
	template<typename T> static DirectX::XMVECTOR SampleTexture2D(Vector2 uv, uint32 arraySlice, const TextureData<T>& texData)
	{
		return SampleTexture2D(uv, arraySlice, texData.Texels, texData.Width, texData.Height, texData.NumSlices);
	}

	template<typename T> static DirectX::XMVECTOR SampleTexture2D(Vector2 uv, const TextureData<T>& texData)
	{
		return SampleTexture2D(uv, 0, texData.Texels, texData.Width, texData.Height, texData.NumSlices);
	}

	template<typename T> static DirectX::XMVECTOR SampleCubemap(Vector2 direction, const TextureData<T>& texData)
	{
		...
		uint32 faceIdx = 0;
		Vector2 uv = Vector2(direction.y, direction.z);
		if (direction.x == maxComponent)
		{
			faceIdx = 0;
			uv = Vector2(-direction.z, -direction.y) / direction.x;
		}
		...
		{
			faceIdx = 2;
			uv = Vector2(direction.x, direction.z) / direction.y;
		}
		else if (-direction.y == maxComponent)
		{
			faceIdx = 3;
		...
		{
			faceIdx = 4;
			uv = Vector2(direction.x, -direction.y) / direction.z;
		}
		else if (-direction.z == maxComponent)
		{
			faceIdx = 5;
		...
		uv = uv * Vector2(0.5f, 0.5f) + Vector2(0.5f, 0.5f);

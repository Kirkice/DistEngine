/* $Header: /common/Render/include/Flitering.h	       8/28/21 13:13pm Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
*						Project Name : DistEngine                                             *
*                                                                                             *
*						File Name : Flitering.h												  *
*                                                                                             *
*                       Programmer : Kirk                                                     *
*                                                                                             *
*---------------------------------------------------------------------------------------------*/
...
namespace render
{
	...
	inline float FilterBox2D(Vector2 xy)
	{
		return FilterBox1D(xy.x) * FilterBox1D(xy.y);
	}

	inline float FilterCircle2D(Vector2 xy)
	{
		return FilterBox1D(Vector2::Length(xy));
	}
	...
	inline float FilterTriangle2D(Vector2 xy)
	{
		return FilterTriangle1D(xy.x) * FilterTriangle1D(xy.y);
	}

	inline float FilterCone2D(Vector2 xy)
	{
		return FilterTriangle1D(Vector2::Length(xy));
	}
	...
	inline float FilterGaussian2D(Vector2 xy, float sigma)
	{
	...
	inline float FilterCubic2D(Vector2 xy, float B, float C)
	{
	...
	inline float FilterBSpline2D(Vector2 xy)
	{
	...
	inline float FilterCatmullRom2D(Vector2 xy)
	{
	...
	inline float FilterMitchell2D(Vector2 xy)
	{
	...
	inline float FilterSinc1D(float x)
	{
		...
		if (x < 0.001f)
			...
			s = std::sin(x * math::DIST_Pi) / (x * math::DIST_Pi);
	...
	inline float BlackmanHarris(float x)
	{
		...
		return a0 - a1 * std::cos(math::DIST_Pi * x) + a2 * std::cos(2 * math::DIST_Pi * x) - a3 * std::cos(3 * math::DIST_Pi * x);
	}
	...
	inline float FilterSmoothstep2D(Vector2 xy)
	{
		return FilterSmoothstep1D(Vector2::Length(xy));
	}
}

/* $Header: /common/Render/include/Spectrum.h			8/28/21 15:23pm Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
namespace render
{
	...
	public:
		...
		CoefficientSpectrum Clamp(float low = 0, float high = FloatInfinity) const {
			CoefficientSpectrum ret;
			for (int i = 0; i < nSpectrumSamples; ++i)
				ret.c[i] = SampleFramework12::Clamp(c[i], low, high);
			Assert_(!ret.HasNaNs());
			return ret;
		}
		bool HasNaNs() const {
	...
	public:
		...
		static SampledSpectrum FromRGB(
			Vector3 rgb, SpectrumType type = SpectrumType::Illuminant) {

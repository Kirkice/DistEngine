/* $Header: /common/Render/include/D3D12Helpers.h	    8/28/21 13:13pm Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
namespace render
{
	...
	{
		// Constants
		const uint64 ConstantBufferAlignment = D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT;		//常量
		const uint64 VertexBufferAlignment = 4;														//顶点
		const uint64 IndexBufferAlignment = 4;														//索引
		const uint32 StandardMSAAPattern = 0xFFFFFFFF;												//MSAA检查

		// Externals
		extern uint32 RTVDescriptorSize;															//RTV(Render Target View)描述符大小
		extern uint32 SRVDescriptorSize;															//SRV(Shader Resource View)描述符大小
		extern uint32 UAVDescriptorSize;															//UAV(Unordered Access View)描述符大小
		extern uint32 CBVDescriptorSize;															//CBV(Constant Buffer View)描述符大小
		extern uint32 DSVDescriptorSize;															//DSV(Depth Stencil View)描述符大小

		extern DescriptorHeap RTVDescriptorHeap;													//RTV描述堆
		extern DescriptorHeap SRVDescriptorHeap;													//SRV描述堆
		extern DescriptorHeap DSVDescriptorHeap;													//DSV描述堆
		extern DescriptorHeap UAVDescriptorHeap;													//UAV描述堆

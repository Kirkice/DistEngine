/* $Header: /common/Render/src/Profiler.cpp				8/21/21 18:26p Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
#include "../../Render/include/Profiler.h"
...
namespace render
{
	...
	void Profiler::Initialize()
	{
		...
		d3d12::Device->CreateQueryHeap(&heapDesc, IID_PPV_ARGS(&queryHeap));

		readbackBuffer.Initialize(MaxProfiles * d3d12::RenderLatency * 2 * sizeof(uint64));
	...
	{
		d3d12::DeferredRelease(queryHeap);
	...
	void Profiler::EndProfile(ID3D12GraphicsCommandList* cmdList, uint64 idx)
	{
		...
		// Resolve the data
		const uint64 dstOffset = ((d3d12::CurrFrameIdx * MaxProfiles * 2) + startQueryIdx) * sizeof(uint64);
	...
	void Profiler::EndFrame(uint32 displayWidth, uint32 displayHeight)
	{
		...
		{
			d3d12::GfxQueue->GetTimestampFrequency(&gpuFrequency);

			const uint64* queryData = readbackBuffer.Map<uint64>();
			frameQueryData = queryData + (d3d12::CurrFrameIdx * MaxProfiles * 2);
		}
	...
	double Profiler::GPUProfileTiming(const char* name) const
	{
		...
		const uint64* queryData = readbackBuffer.Map<uint64>();
		const uint64* frameQueryData = queryData + (d3d12::CurrFrameIdx * MaxProfiles * 2);

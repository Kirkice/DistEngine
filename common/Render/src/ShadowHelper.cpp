/* $Header: /common/Render/src/ShadowHelper.cpp		    8/21/21 18:26p Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
*						Project Name : DistEngine                                             *
*                                                                                             *
*						File Name : ShadowHelper.cpp										  *
*                                                                                             *
*                       Programmer : Kirk                                                     *
*                                                                                             *
*---------------------------------------------------------------------------------------------*/
...
#include "../../Render/include/ShadowHelper.h"
...
namespace render
{
	namespace ShadowHelper
	{
		...
		void Initialize(ShadowMapMode smMode, ShadowMSAAMode msaaMode)
		{
			...
			if (smMode == ShadowMapMode::EVSM || smMode == ShadowMapMode::MSM)
			{
...

				{
					...
					staticSamplers[0] = d3d12::GetStaticSamplerState(SamplerState::LinearClamp, 0);
...

				{
					...
					rootParameters[CSRootParam_StandardDescriptors].DescriptorTable.pDescriptorRanges = d3d12::StandardDescriptorRanges();
					rootParameters[CSRootParam_StandardDescriptors].DescriptorTable.NumDescriptorRanges = d3d12::NumStandardDescriptorRanges;
					...
					d3d12::CreateRootSignature(&rootSignatureCS, rootSignatureDesc);
				}
			}
		...
		void Shutdown()
		{
			...
			d3d12::DeferredRelease(rootSignature);
			d3d12::DeferredRelease(rootSignatureCS);
		...
		void CreatePSOs()
		{
			...
			DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&smConvertPSO)));

			for (uint32 i = 0; i <= MaxFilterRadius; ++i)
			{
				psoDesc.PS = filterSMHorizontalPS[i].ByteCode();
				DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&filterSMHorizontalPSO[i])));

				psoDesc.PS = filterSMVerticalPS[i].ByteCode();
				DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&filterSMVerticalPSO[i])));
			}

			psoDesc.PS = filter3x3PS.ByteCode();
			DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&filter3x3PSO)));

			psoDesc.PS = filter5x5PS.ByteCode();
			DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&filter5x5PSO)));
			...
			for (uint32 i = 0; i <= MaxFilterRadius; ++i)
			{
				...
				DXCall(d3d12::Device->CreateComputePipelineState(&psoDescCS, IID_PPV_ARGS(&smConvertAndFilterPSO[i])));
			}
		}
		...
		void DestroyPSOs()
		{
			...
			d3d12::DeferredRelease(smConvertPSO);
			d3d12::DeferredRelease(filter3x3PSO);
			d3d12::DeferredRelease(filter5x5PSO);
			for (uint32 i = 0; i <= MaxFilterRadius; ++i)
			{
				d3d12::DeferredRelease(filterSMHorizontalPSO[i]);
				d3d12::DeferredRelease(filterSMVerticalPSO[i]);
				d3d12::DeferredRelease(smConvertAndFilterPSO[i]);
			}
		}
			...
			bool32 useCSConversion, bool32 use3x3Filter, float positiveExponent, float negativeExponent)
		{
			...
			if (useCSConversion)
			{
				...
				d3d12::BindStandardDescriptorTable(cmdList, CSRootParam_StandardDescriptors, CmdListMode::Compute);

				d3d12::BindTempConstantBuffer(cmdList, constants, CSRootParam_CBuffer, CmdListMode::Compute);

				D3D12_CPU_DESCRIPTOR_HANDLE uavs[] = { smTarget.UAV };
				d3d12::BindTempDescriptorTable(cmdList, uavs, ArraySize_(uavs), CSRootParam_UAV, CmdListMode::Compute);

				const uint32 BaseThreadGroupWidth = 8;
				cmdList->Dispatch(d3d12::DispatchSize(smTarget.Width(), BaseThreadGroupWidth), d3d12::DispatchSize(smTarget.Height(), BaseThreadGroupWidth), 1);
			...
			else if (use3x3Filter && ((sampleRadiusU == 1 && sampleRadiusV == 1) || (sampleRadiusU == 2 && sampleRadiusV == 2)))
			{
				...
				d3d12::SetViewport(cmdList, tempTarget.Width(), tempTarget.Height());
				...
				d3d12::BindTempConstantBuffer(cmdList, constants, RootParam_CBuffer, CmdListMode::Graphics);
			...
			else
			{
				...
				cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				d3d12::SetViewport(cmdList, smTarget.Width(), smTarget.Height());
			...
			SunShadowConstantsBase& constants, OrthographicCamera* cascadeCameras)
		{
			...
			for (uint64 cascadeIdx = 0; cascadeIdx < NumCascades; ++cascadeIdx)
			{
				...
				if (stabilize)
				{
					...
					for (uint64 i = 0; i < 8; ++i)
					{
						float dist = Vector3::Length(Vector3(frustumCornersWS[i]) - frustumCenter);
				...
				else
				{
					...
					Vector3 lookAt = frustumCenter - lightDir;
					...
					minExtents = Vector3(mins);
					maxExtents = Vector3(maxes);
				}
				...
				else
				{
					...
					cascadeCorner = Vector3::Transform(cascadeCorner, c0Matrix);

					// Do the same for the upper corner
					Vector3 otherCorner = Vector3::Transform(Vector3(1.0f, 1.0f, 1.0f), invCascadeMat);
					otherCorner = Vector3::Transform(otherCorner, c0Matrix);

					// Calculate the scale and offset
					Vector3 cascadeScale = Vector3(1.0f, 1.0f, 1.f) / (otherCorner - cascadeCorner);

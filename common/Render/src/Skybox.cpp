/* $Header: /common/Render/include/SkyBox.h				8/28/21 15:23pm Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
#include "../../Render/include/Skybox.h"
#include "../../Render/include/ArHosekSkyModel.h"
#include "../../Render/include/ShaderCompiler.h"
#include "../../Render/include/Textures.h"
#include "../../Render/include/Spectrum.h"
...
namespace render
{
	...
	static float AngleBetween(const Vector3& dir0, const Vector3& dir1)
	{
		return std::acos(std::max(Vector3::Dot(dir0, dir1), 0.00001f));
	}
	...
	static float IrradianceIntegral(float theta)
	{
		...
		return DIST_Pi * sinTheta * sinTheta;
	}

	bool SkyCache::Init(const Vector3& sunDirection_, float sunSize, const Vector3& groundAlbedo_, float turbidity, bool createCubemap)
	{
		Vector3 sunDirection = sunDirection_;
		Vector3 groundAlbedo = groundAlbedo_;
		...
		float thetaS = AngleBetween(sunDirection, Vector3(0, 1, 0));
		float elevation = DIST_Pi2 - thetaS;
		...
		Vector3 sunDirX = Vector3::Perpendicular(sunDirection);
		Vector3 sunDirY = Vector3::Cross(sunDirection, sunDirX);
		...
		for (uint64 x = 0; x < NumSamples; ++x)
		{
			for (uint64 y = 0; y < NumSamples; ++y)
			{
				...
				Vector3 sampleDir = SampleDirectionCone(u1, u2, CosPhysicalSunSize);
				sampleDir = Vector3::Transform(sampleDir, sunOrientation);

				float sampleThetaS = AngleBetween(sampleDir, Vector3(0, 1, 0));
				float sampleGamma = AngleBetween(sampleDir, sunDirection);

				for (int32 i = 0; i < NumSpectralSamples; ++i)
				{
					float wavelength = math::Lerp(float(SampledLambdaStart), float(SampledLambdaEnd), i / float(NumSpectralSamples));
					solarRadiance[i] = float(arhosekskymodel_solar_radiance(skyStates[i], sampleThetaS, sampleGamma, wavelength));
				}

				Vector3 sampleRadiance = solarRadiance.ToRGB();
				...
				SunIrradiance += sampleRadiance * Saturate(Vector3::Dot(sampleDir, sunDirection));
			}
		}
		...
		if (createCubemap)
		{
			...
			Array<Vector3> samples(NumTexels);
			Array<Vector3> sampleDirs(NumTexels);
			Array<math::Half4> texels(NumTexels);
			...
			for (uint64 s = 0; s < 6; ++s)
			{
				for (uint64 y = 0; y < CubeMapRes; ++y)
				{
					...
					{
						Vector3 dir = MapXYSToDirection(x, y, s, CubeMapRes, CubeMapRes);
						Vector3 radiance = Sample(dir);

						uint64 idx = (s * CubeMapRes * CubeMapRes) + (y * CubeMapRes) + x;
						samples[idx] = radiance;
						texels[idx] = math::Half4(math::Vector4(radiance, 1.0f));
						sampleDirs[idx] = dir;
	...
	Vector3 SkyCache::Sample(Vector3 sampleDir) const
	{
		Assert_(StateR != nullptr);

		float gamma = AngleBetween(sampleDir, SunDirection);
		float theta = AngleBetween(sampleDir, Vector3(0, 1, 0));
	...
	void Skybox::Initialize()
	{
...

		{
			...
			staticSamplers[0] = d3d12::GetStaticSamplerState(SamplerState::LinearClamp);
	...
	void Skybox::CreatePSOs(DXGI_FORMAT rtFormat, DXGI_FORMAT depthFormat, uint32 numMSAASamples)
	{
		...
		psoDesc.DepthStencilState = d3d12::GetDepthState(DepthState::Enabled);
		...
		DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineState)));
	}

	void Skybox::DestroyPSOs()
	{
		d3d12::DeferredRelease(pipelineState);
	}

	void Skybox::RenderCommon(ID3D12GraphicsCommandList* cmdList, const Texture* environmentMap,
		const Float4x4& view, const Float4x4& projection, Vector3 scale)
	{
		// Set pipeline state
		...
		d3d12::BindTempConstantBuffer(cmdList, vsConstants, RootParam_VSCBuffer, CmdListMode::Graphics);

		psConstants.Scale = scale;
		psConstants.EnvMapIdx = environmentMap->SRV;
		d3d12::BindTempConstantBuffer(cmdList, psConstants, RootParam_PSCBuffer, CmdListMode::Graphics);
	...
	void Skybox::RenderEnvironmentMap(ID3D12GraphicsCommandList* cmdList, const Float4x4& view,
		const Float4x4& projection, const Texture* environmentMap, Vector3 scale)
	{
	...
	void Skybox::RenderSky(ID3D12GraphicsCommandList* cmdList, const Float4x4& view,
		...
		bool enableSun, const Vector3& scale)
	{

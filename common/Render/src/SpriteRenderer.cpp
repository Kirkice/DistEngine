/* $Header: /common/Render/include/SpriteRenderer.h		8/28/21 15:23pm Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
#include "../../Render/include/SpriteRenderer.h"
...
namespace render
{
	...
	void SpriteRenderer::CreatePSOs(DXGI_FORMAT rtFormat)
	{
		...
		for (uint64 i = 0; i < uint64(SpriteBlendMode::NumValues); ++i)
		{
			...
			psoDesc.BlendState = d3d12::GetBlendState(blendStates[i]);
			...
			DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineStates[i])));
		}
	}
	...
	void SpriteRenderer::DestroyPSOs()
	{
		for (uint64 i = 0; i < ArraySize_(pipelineStates); ++i)
			d3d12::DeferredRelease(pipelineStates[i]); 
	}

	void SpriteRenderer::Begin(ID3D12GraphicsCommandList* cmdList, Vector2 viewportSize, SpriteFilterMode filterMode, SpriteBlendMode blendMode)
	{
	...
	void SpriteRenderer::Render(ID3D12GraphicsCommandList* cmdList, const Texture* texture, const SpriteTransform& transform,
		const Vector4& color, const Vector4* drawRect)
	{
		if (texture == nullptr)
		...
		if (drawRect != nullptr)
			...
			drawData.DrawRect = math::Vector4(0.0f, 0.0f, float(texture->Width), float(texture->Height));
		...
		const SpriteDrawData* drawData, uint64 numSprites)
	{
		...
		perBatchData.TextureSize = Vector2(float(texture->Width), float(texture->Height));
		d3d12::BindTempConstantBuffer(cmdList, perBatchData, RootParam_PerBatchCB, CmdListMode::Graphics);

#if Debug_
		// Make sure the draw rects are all valid
		for (uint64 i = 0; i < numSprites; ++i)
		{
			Vector4 drawRect = drawData[i].DrawRect;
		...
		for (uint64 offset = 0; offset < numSprites; offset += MaxBatchSize)
		{
			...
			TempBuffer instanceBuffer = d3d12::TempStructuredBuffer(spritesToDraw, sizeof(SpriteDrawData));
			...
			d3d12::BindTempConstantBuffer(cmdList, srvIndices, RootParam_SRVIndicesCB, CmdListMode::Graphics);
	...
	void SpriteRenderer::RenderText(ID3D12GraphicsCommandList* cmdList, const SpriteFont& font,
		const wchar* text, Vector2 position, const Vector4& color)
	{

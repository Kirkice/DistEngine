/* $Header: /common/core/include/Settings.cpp					8/11/21 20:55p Kirk			$ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
namespace core
{
    static Vector2 CanvasTransform(Vector2 pos, Vector2 canvasStart, Vector2 canvasSize)
    {
        return canvasStart + canvasSize * (pos * Vector2(0.5f, -0.5f) + Vector2(0.5f)); 
    }
    ...
    void DirectionSetting::Initialize(const char* name_, const char* group_, const char* label_,
        const char* helpText_, Vector3 initialVal, bool convertToViewSpace_)
    {
        val = Vector3::Normalize(initialVal);
        oldVal = val;
        spherical = math::CartesianToSpherical(val); 
    ...
    void DirectionSetting::Update(const Float4x4& viewMatrix)
    {
        ...
        {
            Vector2 dragDelta = ToFloat2(ImGui::GetMouseDragDelta());
            Vector2 rotAmt = dragDelta;
            ...
            {
                Vector3 dirVS = Vector3::TransformDirection(val, viewMatrix);
                dirVS = Vector3::Transform(dirVS, rotation);
                val = Vector3::TransformDirection(dirVS, Float4x4::Transpose(viewMatrix));
            }
            else
                val = Vector3::Transform(val, rotation); 
        ...
        Vector2 canvasStart = ToFloat2(ImGui::GetItemRectMin());
        Vector2 canvasSize = ToFloat2(ImGui::GetItemRectSize());
        Vector2 canvasEnd = ToFloat2(ImGui::GetItemRectMax());
        ...
        Vector3 drawDir = val;
        if (convertToViewSpace)
            drawDir = Vector3::TransformDirection(val, viewMatrix);
        ...
        basis.SetXBasis(Vector3::Normalize(Vector3::Cross(up, drawDir)));
        basis.SetYBasis(Vector3::Cross(drawDir, basis.Right()));

        const float arrowHeadSize = 0.2f;
        Vector3 arrowHeadPoints[] =
        {
            Vector3(0.0f, 0.0f, 1.0f) + Vector3(1.0f, 0.0f, -1.0f) * arrowHeadSize,
            Vector3(0.0f, 0.0f, 1.0f) + Vector3(0.0f, 1.0f, -1.0f) * arrowHeadSize,
            Vector3(0.0f, 0.0f, 1.0f) + Vector3(-1.0f, 0.0f, -1.0f) * arrowHeadSize,
            Vector3(0.0f, 0.0f, 1.0f) + Vector3(0.0f, -1.0f, -1.0f) * arrowHeadSize,
        };

        Vector2 startPoint = CanvasTransform(0.0f, canvasStart, canvasSize);
        ImColor color = ImColor(1.0f, 1.0f, 0.0f);

        Vector2 endPoint = CanvasTransform(drawDir.To2D(), canvasStart, canvasSize);
        drawList->AddLine(ToImVec2(startPoint), ToImVec2(endPoint), color);

        for (uint64 i = 0; i < ArraySize_(arrowHeadPoints); ++i)
        {
            Vector2 headPoint = Vector3::Transform(arrowHeadPoints[i], basis).To2D();
            ...
            Vector2 nextHeadPoint = Vector3::Transform(arrowHeadPoints[nextHeadIdx], basis).To2D();
        ...
        {
            Vector2 degrees = Vector2(RAD2DEG(spherical.x), RAD2DEG(spherical.y));
            ...
            if (sliderChanged)
            {
                spherical = Vector2(DEG2RAD(degrees.x), DEG2RAD(degrees.y));
    ...
    Vector3 DirectionSetting::Value() const
    {
        return val;
    }

    void DirectionSetting::SetValue(Vector3 newVal)
    {
        val = Vector3::Normalize(newVal);
    }

    DirectionSetting::operator Vector3()
    {
        return val;
    }
    ...
    void OrientationSetting::Update(const Float4x4& viewMatrix)
    {
        ...
        Vector2 endPoints[3] = { basis.Right().To2D(), basis.Up().To2D(), basis.Forward().To2D() };
        Vector3 colors[3] = { Vector3(1.0f, 0.0, 0.0f), Vector3(0.0f, 1.0f, 0.0f), Vector3(0.0f, 0.0f, 1.0f) };

        for (uint64 i = 0; i < 3; ++i)
        {
            Vector2 endPoint = CanvasTransform(endPoints[i], canvasStart, canvasSize);
    ...
    void ColorSetting::Initialize(const char* name_,
        const char* group_, const char* label_,
        const char* helpText_, Vector3 initialVal,
        ...
        float step_, ColorUnit units_)
    {
        ...
        if (hdr)
        {
            val = Vector3::Clamp(initialVal, 0.0f, FLT_MAX);
            ...
            val = Vector3::Clamp(initialVal, 0.0f, 1.0f);
    ...
    void ColorSetting::Update(const Float4x4& viewMatrix)
    {
        ...
        Vector3 newVal = val * multiplier;
        changed = oldVal != newVal;
        oldVal = newVal;
    }

    Vector3 ColorSetting::Value() const
    {
        return oldVal;
    }

    void ColorSetting::SetValue(Vector3 newVal)
    {
        if (hdr)
        {
            newVal = Vector3::Clamp(newVal, 0.0f, FLT_MAX);
            float newIntensity = std::max(std::max(newVal.x, newVal.y), newVal.z);
            newVal /= newIntensity;
            intensity.SetValue(newIntensity);
        }

        val = Vector3::Clamp(newVal, 0.0f, 1.0f);
    }

    ColorSetting::operator Vector3()
    {
        return Value();
    }

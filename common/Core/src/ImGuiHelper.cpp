/* $Header: /common/core/include/ImGuiHelper.cpp				8/11/21 20:55p Kirk			$ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
#include "../../Core/include/ImGuiHelper.h"
...
namespace core
{
	namespace ImGuiHelper
	{
		...
		void CreatePSOs(DXGI_FORMAT rtFormat)
		{
			...
			psoDesc.BlendState = d3d12::GetBlendState(BlendState::AlphaBlend);
...
#if UseAsserts_
			CurrBeginFrame = d3d12::CurrentCPUFrame;
#endif
		}
		...
		{
			Assert_(CurrBeginFrame == d3d12::CurrentCPUFrame);
			Assert_(CurrEndFrame != d3d12::CurrentCPUFrame);
			...
			MapResult vertexMem = d3d12::AcquireTempBufferMem(vbSize, 4);
			MapResult indexMem = d3d12::AcquireTempBufferMem(ibSize, 4);
...

			{
				...
				constants.ProjectionMatrix = Float4x4(math::Vector4(2.0f / (R - L), 0.0f, 0.0f, 0.0f),
					math::Vector4(0.0f, 2.0f / (T - B), 0.0f, 0.0f),
					math::Vector4(0.0f, 0.0f, 0.5f, 0.0f),
					math::Vector4((R + L) / (L - R), (T + B) / (B - T), 0.5f, 1.0f));
			}

			// Setup viewport
			d3d12::SetViewport(cmdList, displayWidth, displayHeight);
...
#if UseAsserts_
			CurrEndFrame = d3d12::CurrentCPUFrame;
#endif
		}

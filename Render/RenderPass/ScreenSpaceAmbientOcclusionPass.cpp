...
namespace Dist
{
    ...
    ID3D12Resource* SsaoPass::NormalMap()
    {
        return mNormalMap.Get();
    }

    ID3D12Resource* SsaoPass::AmbientMap()
    {
        return mAmbientMap0.Get();
    }

    CD3DX12_CPU_DESCRIPTOR_HANDLE SsaoPass::NormalMapRtv()const
    {
        return mhNormalMapCpuRtv;
    }

    CD3DX12_GPU_DESCRIPTOR_HANDLE SsaoPass::NormalMapSrv()const
    {
        return mhNormalMapGpuSrv;
    }

    CD3DX12_GPU_DESCRIPTOR_HANDLE SsaoPass::AmbientMapSrv()const
    {
    ...
    {
        // Save references to the descriptors.  The SsaoPass reserves heap space
    ...
    void SsaoPass::RebuildDescriptors(ID3D12Resource* depthStencilBuffer)
    {
    ...
    void SsaoPass::SetPSOs(ID3D12PipelineState* ssaoPso, ID3D12PipelineState* ssaoBlurPso)
    {
        mSsaoPso = ssaoPso;
    ...
    void SsaoPass::ComputeSsao(
        ID3D12GraphicsCommandList* cmdList,
        DefaultFrameResource* currFrame,
        int blurCount)
    {
    ...
    void SsaoPass::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, DefaultFrameResource* currFrame, int blurCount)
    {
    ...
    void SsaoPass::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, bool horzBlur)
    {
    ...
    void SsaoPass::BuildResources()
    {
        ...
        texDesc.MipLevels = 1;
        texDesc.Format = SsaoPass::NormalMapFormat;
        ...
        texDesc.Format = SsaoPass::AmbientMapFormat;
    ...
    void SsaoPass::BuildRandomVectorTexture(ID3D12GraphicsCommandList* cmdList)
    {
        D3D12_RESOURCE_DESC texDesc;
        ZeroMemory(&texDesc, sizeof(D3D12_RESOURCE_DESC));
        texDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
        ...
        for (int i = 0; i < 256; ++i)
        {
            for (int j = 0; j < 256; ++j)
            {
                ...
                XMFLOAT3 v(Dist::RandF(), Dist::RandF(), Dist::RandF());

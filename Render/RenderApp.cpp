...
namespace Dist
{
	...
	bool RenderApp::Initialize()
	{
		...
		//	初始化SceneRender
		m_SceneRender = DefaultSceneRender();
		//m_SceneRender.mDefaultScene = default_scene;
...
//		m_SceneRender.mDefaultScene.mCurrFrameResourceIndex = (m_SceneRender.mDefaultScene.mCurrFrameResourceIndex + 1) % gNumFrameResources;
//		m_SceneRender.mDefaultScene.mCurrFrameResource = m_SceneRender.mDefaultScene.mFrameResources[m_SceneRender.mDefaultScene.mCurrFrameResourceIndex].get();
//
//		// Has the GPU finished processing the commands of the current frame resource?
//		// If not, wait until the GPU has completed commands up to this fence point.
//		if (m_SceneRender.mDefaultScene.mCurrFrameResource->Fence != 0 && mFence->GetCompletedValue() < m_SceneRender.mDefaultScene.mCurrFrameResource->Fence)
//		{
//			HANDLE eventHandle = CreateEventEx(nullptr, false, false, EVENT_ALL_ACCESS);
//			ThrowIfFailed(mFence->SetEventOnCompletion(m_SceneRender.mDefaultScene.mCurrFrameResource->Fence, eventHandle));
...
//		auto cmdListAlloc = m_SceneRender.mDefaultScene.mCurrFrameResource->CmdListAlloc;
//
...
//		mCommandList->SetGraphicsRootSignature(m_SceneRender.mDefaultScene.mRootSignature.Get());
//		auto matBuffer = m_SceneRender.mDefaultScene.mCurrFrameResource->PBRMaterialBuffer->Resource();
//
//		ThrowIfFailed(mCommandList->Close());
//		ID3D12CommandList* cmdsLists[] = { mCommandList.Get() };
//		mCommandQueue->ExecuteCommandLists(_countof(cmdsLists), cmdsLists);
...
//
//		m_SceneRender.mDefaultScene.mCurrFrameResource->Fence = ++mCurrentFence;
//

...
/* $Header: /Enigne/DistEngine.cpp					8/21/21 18:26p Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
*						Project Name : DistEngine                                             *
*                                                                                             *
*						File Name : DistEngine.cpp											  *
*                                                                                             *
*                       Programmer : Kirk                                                     *
*                                                                                             *
*---------------------------------------------------------------------------------------------*/
...
#include "../common/Core/include/Window.h"
#include "../common/Core/include/Input.h"
#include "../common/Core/include/Utility.h"
#include "../common/Render/include/SwapChain.h"
#include "../common/Render/include/ShaderCompiler.h"
#include "../common/Render/include/Profiler.h"
#include "../common/Render/include/Textures.h"
#include "../common/Render/include/Sampling.h"
#include "../common/Render/include/D3D12.h"
#include "../common/Render/include/D3D12Helpers.h"
#include "../common/Render/include/DXRHelper.h"
#include "../common/Render/include/BRDF.h"
#include "../common/Core/include/TaskScheduler_c.h"
#include "../common/ImGUI/include/imgui.h"
#include "../common/Core/include/ImGuiHelper.h"
...
static const Vector3 SceneCameraPositions[] = { Vector3(-11.5f, 1.85f, -0.45f), Vector3(-1.0f, 5.5f, 12.0f), Vector3(0.0f, 2.5f, -10.0f), Vector3(0.0f, 0.0f, -3.0f) };
static const Vector2 SceneCameraRotations[] = { Vector2(0.0f, 1.544f), Vector2(0.2f, 3.0f), Vector2(0.0f, 0.0f), Vector2(0.0f, 0.0f) };
static const Vector3 SceneSunDirections[] = { Vector3(0.26f, 0.987f, -0.16f), Vector3(-0.133022308f, 0.642787635f, 0.75440651f), Vector3(0.26f, 0.987f, -0.16f), Vector3(0.0f, 1.0f, 0.0f) }; 
...
static bool SphereConeIntersection(const Vector3& coneTip, const Vector3& coneDir, float coneHeight,
	float coneAngle, const Vector3& sphereCenter, float sphereRadius)
{
	if (Vector3::Dot(sphereCenter - coneTip, coneDir) > coneHeight + sphereRadius)
		return false;
	...
	float a = Vector3::Dot(v, coneDir);
	float b = a * sinHalfAngle / cosHalfAngle;
	float c = std::sqrt(Vector3::Dot(v, v) - a * a);
	float d = c - b;
...
DistEngine::DistEngine(const wchar* cmdLine) : App(L"DXR Path Tracer", cmdLine)
{
...
void DistEngine::BeforeReset()
{
}

void DistEngine::AfterReset()
{
...
void DistEngine::Initialize()
{
	...
	d3d12::Device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &features, sizeof(features));
	...
	float aspect = float(swapChain.Width()) / swapChain.Height();
	camera.Initialize(aspect, math::DIST_Pi_4, 0.1f, 100.0f);
...

	{
		...
		rootParameters[ClusterParams_StandardDescriptors].DescriptorTable.pDescriptorRanges = d3d12::StandardDescriptorRanges();
		rootParameters[ClusterParams_StandardDescriptors].DescriptorTable.NumDescriptorRanges = d3d12::NumStandardDescriptorRanges;
		...
		d3d12::CreateRootSignature(&clusterRS, rootSignatureDesc);
	}

	{
		...
		rootParameters[ResolveParams_StandardDescriptors].DescriptorTable.pDescriptorRanges = d3d12::StandardDescriptorRanges();
		rootParameters[ResolveParams_StandardDescriptors].DescriptorTable.NumDescriptorRanges = d3d12::NumStandardDescriptorRanges;
		...
		d3d12::CreateRootSignature(&resolveRootSignature, rootSignatureDesc);
	}
...
void DistEngine::Shutdown()
{
	...
	d3d12::Release(resolveRootSignature);

	rtTarget.Shutdown();
	d3d12::Release(rtRootSignature);
...
void DistEngine::CreatePSOs()
{
...

	{
		...
		if (rastMode == ClusterRasterizationModes::MSAA4x || rastMode == ClusterRasterizationModes::MSAA8x)
		{
			...
			psoDesc.SampleDesc.Quality = d3d12::StandardMSAAPattern;
		...
		psoDesc.RasterizerState = d3d12::GetRasterizerState(RasterizerState::BackFaceCull);
		psoDesc.RasterizerState.ConservativeRaster = crMode;
		DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&clusterFrontFacePSO)));

		psoDesc.PS = clusterBackFacePS.ByteCode();
		psoDesc.RasterizerState = d3d12::GetRasterizerState(RasterizerState::FrontFaceCull);
		psoDesc.RasterizerState.ConservativeRaster = crMode;
		DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&clusterBackFacePSO)));

		psoDesc.PS = clusterIntersectingPS.ByteCode();
		psoDesc.RasterizerState = d3d12::GetRasterizerState(RasterizerState::FrontFaceCull);
		psoDesc.RasterizerState.ConservativeRaster = crMode;
		DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&clusterIntersectingPSO)));
	...
	if (msaaEnabled)
	{
		...
		DXCall(d3d12::Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&resolvePSO)));
	}

	CreateRayTracingPSOs();
}

void DistEngine::DestroyPSOs()
{
	meshRenderer.DestroyPSOs();
	skybox.DestroyPSOs();
	postProcessor.DestroyPSOs();
	d3d12::DeferredRelease(clusterFrontFacePSO);
	d3d12::DeferredRelease(clusterBackFacePSO);
	d3d12::DeferredRelease(clusterIntersectingPSO);
	d3d12::DeferredRelease(resolvePSO);

	d3d12::DeferredRelease(rtPSO);
}

// Creates all required render targets
void DistEngine::CreateRenderTargets()
{
...
void DistEngine::InitializeScene()
{
...
void DistEngine::InitRayTracing()
{
...

	{
		...
		rootParameters[RTParams_StandardDescriptors].DescriptorTable.pDescriptorRanges = d3d12::StandardDescriptorRanges();
		rootParameters[RTParams_StandardDescriptors].DescriptorTable.NumDescriptorRanges = d3d12::NumStandardDescriptorRanges;
		...
		staticSamplers[0] = d3d12::GetStaticSamplerState(SamplerState::Anisotropic, 0, 0, D3D12_SHADER_VISIBILITY_ALL);
		staticSamplers[1] = d3d12::GetStaticSamplerState(SamplerState::LinearClamp, 1, 0, D3D12_SHADER_VISIBILITY_ALL);
		...
		d3d12::CreateRootSignature(&rtRootSignature, rootSignatureDesc);
	}

	rtCurrCamera = camera;
}

void DistEngine::CreateRayTracingPSOs()
{
	...
	d3d12::Release(psoProps);
}

void DistEngine::Update(const Timer& timer)
{
	...
	{
		d3d12::Device->SetStablePowerState(AppSettings::StablePowerState);
...
void DistEngine::Render(const Timer& timer)
{
	...
	d3d12::SetViewport(cmdList, swapChain.Width(), swapChain.Height());

	RenderHUD(timer);
}

void DistEngine::UpdateLights()
{
	...
	const float inRadius = std::cos(math::DIST_Pi / NumConeSides);
	...
	Vector3 nearClipCenter = cameraPos + nearClip * camera.Forward();
	Float4x4 invViewProjection = Float4x4::Invert(camera.ViewProjectionMatrix());
	Vector3 nearTopRight = Vector3::Transform(Vector3(1.0f, 1.0f, 0.0f), invViewProjection);
	float nearClipRadius = Vector3::Length(nearTopRight - nearClipCenter);
	...
	for (uint64 spotLightIdx = 0; spotLightIdx < numSpotLights; ++spotLightIdx)
	{
		...
		{
			Vector3 coneVert = coneVertices[i] * bounds.Scale;
			coneVert = Vector3::Transform(coneVert, bounds.Orientation);
			coneVert += bounds.Position;

			float vertZ = Vector3::Transform(coneVert, viewMatrix).z;
			minZ = Min(minZ, vertZ);
			maxZ = Max(maxZ, vertZ);
		}
...
void DistEngine::RenderClusters()
{
	ID3D12GraphicsCommandList* cmdList = d3d12::CmdList;
...

	{
		...
		D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = d3d12::TempDescriptorTable(cpuDescriptors, ArraySize_(cpuDescriptors));
	...
	d3d12::SetViewport(cmdList, AppSettings::NumXTiles, AppSettings::NumYTiles);
	cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	cmdList->SetGraphicsRootSignature(clusterRS);

	d3d12::BindStandardDescriptorTable(cmdList, ClusterParams_StandardDescriptors, CmdListMode::Graphics);
	...
	if (AppSettings::RenderLights)
	{
		...
		d3d12::BindTempConstantBuffer(cmdList, clusterConstants, ClusterParams_CBuffer, CmdListMode::Graphics);

		AppSettings::BindCBufferGfx(cmdList, ClusterParams_AppSettings);

		D3D12_CPU_DESCRIPTOR_HANDLE uavs[] = { spotLightClusterBuffer.UAV };
		d3d12::BindTempDescriptorTable(cmdList, uavs, ArraySize_(uavs), ClusterParams_UAVDescriptors, CmdListMode::Graphics);
...
void DistEngine::RenderForward()
{
	...
	d3d12::SetViewport(cmdList, mainTarget.Width(), mainTarget.Height());
...
void DistEngine::RenderResolve()
{
	...
	d3d12::SetViewport(cmdList, resolveTarget.Width(), resolveTarget.Height());

	cmdList->SetGraphicsRootSignature(resolveRootSignature);
	cmdList->SetPipelineState(resolvePSO);

	d3d12::BindStandardDescriptorTable(cmdList, ResolveParams_StandardDescriptors, CmdListMode::Graphics);
...
void DistEngine::RenderRayTracing()
{
	...
	d3d12::BindStandardDescriptorTable(cmdList, RTParams_StandardDescriptors, CmdListMode::Compute);

	cmdList->SetComputeRootShaderResourceView(RTParams_SceneDescriptor, rtTopLevelAccelStructure.GPUAddress);
	d3d12::BindTempDescriptorTable(cmdList, &rtTarget.UAV, 1, RTParams_UAVDescriptor, CmdListMode::Compute);
	...
	rtConstants.CosSunAngularRadius = std::cos(math::DEG2RAD(AppSettings::SunSize));
	rtConstants.SinSunAngularRadius = std::sin(math::DEG2RAD(AppSettings::SunSize));
	...
	d3d12::BindTempConstantBuffer(cmdList, rtConstants, RTParams_CBuffer, CmdListMode::Compute);
	...
	d3d12::CmdList->DispatchRays(&dispatchDesc);
...
void DistEngine::RenderHUD(const Timer& timer)
{
	ID3D12GraphicsCommandList* cmdList = d3d12::CmdList;
	...
	Vector2 textPos = Vector2(25.0f, 25.0f);
	std::wstring fpsText = MakeString(L"Frame Time: %.2fms (%u FPS)", 1000.0f / fps, fps);
	spriteRenderer.RenderText(cmdList, font, fpsText.c_str(), textPos, math::Vector4(1.0f, 1.0f, 0.0f, 1.0f));
	...
	if (rtCurrSampleIdx < totalNumSamples && AppSettings::ShowProgressBar)
	{
		...
		Vector2 barStart = Vector2(width * (1.0f - barPercentage) * 0.5f, height - 200.0f);
		Vector2 barSize = Vector2(width * barPercentage, barHeight);
		Vector2 barEnd = barStart + barSize;

		Vector2 windowStart = barStart - 8.0f;
		Vector2 windowSize = barSize + 16.0f;
		...
		Vector2 progressTextSize = ToFloat2(ImGui::CalcTextSize(progressText.c_str()));
		Vector2 progressTextPos = barStart + (barSize * 0.5f) - (progressTextSize * 0.5f);
...
void DistEngine::BuildRTAccelerationStructure()
{
...

	{
		...
		d3d12::Device->GetRaytracingAccelerationStructurePrebuildInfo(&prebuildInfoDesc, &topLevelPrebuildInfo);
	}
...

	{
		...
		d3d12::Device->GetRaytracingAccelerationStructurePrebuildInfo(&prebuildInfoDesc, &bottomLevelPrebuildInfo);
	}
	...
	TempBuffer instanceBuffer = d3d12::TempStructuredBuffer(1, sizeof(D3D12_RAYTRACING_INSTANCE_DESC), false);
	...
	{
		ProfileBlock profileBlock(d3d12::CmdList, "Build Acceleration Structure");

		d3d12::CmdList->BuildRaytracingAccelerationStructure(&bottomLevelBuildDesc, 0, nullptr);
		rtBottomLevelAccelStructure.UAVBarrier(d3d12::CmdList);

		d3d12::CmdList->BuildRaytracingAccelerationStructure(&topLevelBuildDesc, 0, nullptr);
		rtTopLevelAccelStructure.UAVBarrier(d3d12::CmdList);
	}
	...
	lastBuildAccelStructureFrame = d3d12::CurrentCPUFrame;
}

int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	DistEngine app(lpCmdLine);
	app.Run();
}
...
/* $Header: /Enigne/MeshRenderer.cpp					8/21/21 18:26p Kirk 			    $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
#include "../common/Core/include/Expections.h"
...
{
	Vector3 mins = Vector3(camera.MinX(), camera.MinY(), camera.NearClip());
	Vector3 maxes = Vector3(camera.MaxX(), camera.MaxY(), camera.FarClip());
	if (ignoreNearZ)
		mins.z = -10000.0f;

	Vector3 extents = (maxes - mins) / 2.0f;
	Vector3 center = mins + extents;
	center = Vector3::Transform(center, camera.Orientation());
...
void MeshRenderer::Initialize(const Model* model_)
{
	...
	for (uint64 i = 0; i < numMeshes; ++i)
	{
		...
		Vector3 extents = (mesh.AABBMax() - mesh.AABBMin()) / 2.0f;
		Vector3 center = mesh.AABBMin() + extents;
...

	{
		...
		rootParameters[MainPass_StandardDescriptors].DescriptorTable.pDescriptorRanges = d3d12::StandardDescriptorRanges();
		rootParameters[MainPass_StandardDescriptors].DescriptorTable.NumDescriptorRanges = d3d12::NumStandardDescriptorRanges;
		...
		staticSamplers[0] = d3d12::GetStaticSamplerState(SamplerState::Anisotropic, 0);
		staticSamplers[1] = d3d12::GetStaticSamplerState(SamplerState::LinearClamp, 1);
		staticSamplers[2] = d3d12::GetStaticSamplerState(SamplerState::ShadowMapPCF, 2);
		...
		d3d12::CreateRootSignature(&mainPassRootSignature, rootSignatureDesc);
	}

	{
		...
		d3d12::CreateRootSignature(&depthRootSignature, rootSignatureDesc);
	}
}
...
void MeshRenderer::Shutdown()
{
	...
	d3d12::Release(mainPassRootSignature);
	d3d12::Release(depthRootSignature);
}
...
void MeshRenderer::CreatePSOs(DXGI_FORMAT mainRTFormat, DXGI_FORMAT depthFormat, uint32 numMSAASamples)
{
...

	{
		...
		psoDesc.DepthStencilState = d3d12::GetDepthState(DepthState::WritesEnabled);
		...
		psoDesc.SampleDesc.Quality = numMSAASamples > 1 ? d3d12::StandardMSAAPattern : 0;
...

	{
		...
		psoDesc.SampleDesc.Quality = spotLightDepthMap.MSAASamples > 1 ? d3d12::StandardMSAAPattern : 0;
		psoDesc.RasterizerState = d3d12::GetRasterizerState(RasterizerState::BackFaceCull);
		...
		psoDesc.SampleDesc.Quality = sunDepthMap.MSAASamples > 1 ? d3d12::StandardMSAAPattern : 0;
		psoDesc.RasterizerState = d3d12::GetRasterizerState(RasterizerState::BackFaceCullNoZClip);
...
{
	d3d12::DeferredRelease(mainPassPSO);
	d3d12::DeferredRelease(mainPassAlphaTestPSO);
	d3d12::DeferredRelease(depthPSO);
	d3d12::DeferredRelease(spotLightShadowPSO);
	d3d12::DeferredRelease(sunShadowPSO);
}
...
void MeshRenderer::RenderMainPass(ID3D12GraphicsCommandList* cmdList, const Camera& camera, const MainPassData& mainPassData)
{
	...
	d3d12::BindStandardDescriptorTable(cmdList, MainPass_StandardDescriptors, CmdListMode::Graphics);
	...
	d3d12::BindTempConstantBuffer(cmdList, vsConstants, MainPass_VSCBuffer, CmdListMode::Graphics);
	...
	psConstants.CosSunAngularRadius = std::cos(math::DEG2RAD(AppSettings::SunSize));
	psConstants.SinSunAngularRadius = std::sin(math::DEG2RAD(AppSettings::SunSize));
	...
	d3d12::BindTempConstantBuffer(cmdList, psConstants, MainPass_PSCBuffer, CmdListMode::Graphics);

	d3d12::BindTempConstantBuffer(cmdList, sunShadowConstants, MainPass_ShadowCBuffer, CmdListMode::Graphics);
	...
	d3d12::BindTempConstantBuffer(cmdList, psSRVs, MainPass_SRVIndices, CmdListMode::Graphics);
...
void MeshRenderer::RenderDepth(ID3D12GraphicsCommandList* cmdList, const Camera& camera, ID3D12PipelineState* pso, uint64 numVisible, const uint32* meshDrawIndices)
{
	...
	vsConstants.View = camera.ViewMatrix();
	vsConstants.WorldViewProjection = world * camera.ViewProjectionMatrix();
	d3d12::BindTempConstantBuffer(cmdList, vsConstants, 0, CmdListMode::Graphics);
...
void MeshRenderer::RenderSunShadowMap(ID3D12GraphicsCommandList* cmdList, const Camera& camera)
{
	...
	for (uint64 cascadeIdx = 0; cascadeIdx < NumCascades; ++cascadeIdx)
	{
		...
		// Set the viewport
		d3d12::SetViewport(cmdList, SunShadowMapSize, SunShadowMapSize);
...
void MeshRenderer::RenderSpotLightShadowMap(ID3D12GraphicsCommandList* cmdList, const Camera& camera)
{
	...
	for (uint64 i = 0; i < numSpotLights; ++i)
	{
		...
		// Set the viewport
		d3d12::SetViewport(cmdList, SpotLightShadowMapSize, SpotLightShadowMapSize); 

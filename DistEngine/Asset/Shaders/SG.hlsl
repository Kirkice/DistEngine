/* $Header: /PBR/SG.hlsl                                              1/3/22 20:55p KirkZhu $ */
/*--------------------------------------------------------------------------------------------*
*                                                                                             *
...
//-------------------------------------------------------------------------------------------------
SG MakeNormalizedSG(in float3 axis, in float sharpness)
{
    SG sg;
    sg.Axis = axis;
...
SG ConvolveSGs(in SG x, in SG y)
{
    SG res;
    res.Axis = x.Axis;
    res.Sharpness = (x.Sharpness * y.Sharpness) / (x.Sharpness + y.Sharpness);
    res.Amplitude = 2 * PI * x.Amplitude * y.Amplitude / (x.Sharpness + y.Sharpness);
                          ...
                          in float3 view, in float3 specAlbedo)
{
    ...
    output *= GGXV1(m2, nDotL) * GGXV1(m2, nDotV);

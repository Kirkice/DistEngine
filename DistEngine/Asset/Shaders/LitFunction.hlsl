...
inline void InitializeStandardLitSurfaceData(float2 uv, float3 N, float3 T, out SurfaceData outSurfaceData)
{
	...
	float3 bumpedNormalW                                = NormalSampleToWorldSpace(normalMapSample.rgb, normalize(N), T, matData.NormalScale);
    ...
    outSurfaceData.Normal                               = bumpedNormalW * matData.UseNormalMap + (1 - matData.UseNormalMap) * N;

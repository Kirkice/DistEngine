...
void GraphicsCore::BuildDescriptorHeaps()
{
	...
	auto diffuseIBL = mTextures["DiffuseIBL"]->Resource;
...
void GraphicsCore::BuildMaterials()
{
	...
	plane->BaseColor = Vector4(0.93f, 0.81f, 0.56f, 1.0f);
...
void GraphicsCore::BuildRenderItems()
{
	...
	XMMATRIX rotMatrix = XMMatrixRotationX((TargetRotationAngle[0] / 180) * Mathf::Pi) * XMMatrixRotationY((TargetRotationAngle[1] / 180) * Mathf::Pi) * XMMatrixRotationZ((TargetRotationAngle[2] / 180) * Mathf::Pi);
	XMStoreFloat4x4(&sphereRitem->World, XMMatrixScaling(TargetScale[0], TargetScale[1], TargetScale[2]) * XMMatrixTranslation(TargetTransform[0], TargetTransform[1], TargetTransform[2]) * rotMatrix);
	...
	//XMStoreFloat4x4(&gridRitem->TexTransform, XMMatrixScaling(0.0f, 0.0f, 0.0f));
